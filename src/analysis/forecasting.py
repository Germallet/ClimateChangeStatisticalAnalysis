...
temperature_df = surface_temperature_change.dataframe(to_year=2022) # Data from 1850 to 2023
# Date filter
temperature_df = temperature_df.resample("Y").mean(numeric_only=True)
...
forecast_index = pd.date_range(start=train.index[-1], periods=forecast_steps, freq='Y')
...
print(forecast.predicted_mean.value_counts())